...
export function getRecommend () {
  // 线上环境地址，根据实际上线配置修改
  const url = debug ? '/api/getTopBanner' : 'http://120.79.81.194/music/api/getTopBanner'
...
export function getDiscList () {
  // 线上环境地址，根据实际上线配置修改
  const url = debug ? '/api/getDiscList' : 'http://120.79.81.194/music/api/getDiscList'
...
export function getSongList (disstid) {
  const url = debug ? '/api/getCdInfo' : 'http://120.79.81.194/music/api/getCdInfo'
